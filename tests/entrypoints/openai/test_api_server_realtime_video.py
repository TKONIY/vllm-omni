...
def test_realtime_video_serving_factory_is_model_agnostic():
    engine = _FakeDiffusionEngine()

    serving = _create_realtime_video_serving(engine, "robbyant/lingbot-world-fast")

    assert isinstance(serving, RealtimeVideoServing)
    assert serving.__class__ is RealtimeVideoServing
