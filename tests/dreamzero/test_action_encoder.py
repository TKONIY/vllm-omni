...
def _load_dz_classes():
    ...
    import pathlib
    src_path = pathlib.Path(os.path.join(
    ...
    start = next(i for i, l in enumerate(lines) if "class CategorySpecificLinear" in l)
    end = next(i for i, l in enumerate(lines) if "def causal_rope_action_apply" in l)
...
def test_sinusoidal_positional_encoding():
    ...
    for dim in [64, 128, 256, 5120]:
        ...
        assert torch.allclose(vllm_out, dz_out, atol=1e-6), \
            f"SinusoidalPE dim={dim}: max_diff={max_diff}"
...
def test_category_specific_linear():
    ...
    max_diff = (vllm_out - dz_out).abs().max().item()
    assert torch.allclose(vllm_out, dz_out, atol=1e-6), \
        f"CategorySpecificLinear: max_diff={max_diff}"
...
def test_category_specific_mlp():
    ...
    max_diff = (vllm_out - dz_out).abs().max().item()
    assert torch.allclose(vllm_out, dz_out, atol=1e-6), \
        f"CategorySpecificMLP: max_diff={max_diff}"
...
def test_multi_embodiment_action_encoder():
    ...
    max_diff = (vllm_out - dz_out).abs().max().item()
    assert torch.allclose(vllm_out, dz_out, atol=1e-6), \
        f"MultiEmbodimentActionEncoder: max_diff={max_diff}"
