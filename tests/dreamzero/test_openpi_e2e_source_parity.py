...
- non-TP (`nproc_per_node=1` on upstream, single-GPU `vllm serve`)
...
pytestmark = [
    ...
    pytest.mark.skipif(dreamzero_client is None or WebsocketClientPolicy is None,
                       reason="DreamZero client modules are required on PYTHONPATH"),
    pytest.mark.skipif(not DREAMZERO_REPO.exists(),
                       reason="DreamZero source repo is required at ~/code/dreamzero"),
    pytest.mark.skipif(not CHECKPOINT_DIR.exists(),
                       reason="DreamZero local checkpoint is required"),
]
...
def _pick_test_gpu() -> str:
    override = os.environ.get("OPENPI_E2E_GPU")
...
def _run_upstream_service(port: int, log_path: Path) -> subprocess.Popen[str]:
    ...
    env["CUDA_VISIBLE_DEVICES"] = _pick_test_gpu()
    ...
    argv = _torchrun_argv(
        str(Path("tests/dreamzero/upstream_socket_server_no_compile.py")),
        port,
    )
...
def _wait_for_client_ready(client_factory, timeout_s: float, proc=None, log_path: Path | None = None):
    ...
    while time.time() < deadline:
        if proc is not None and proc.poll() is not None:
            ...
            raise RuntimeError(
                f"Service exited before becoming ready with code {proc.returncode}.\n{details}"
            )
        try:
