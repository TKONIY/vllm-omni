...
class FakeDreamZeroImageEncoder(nn.Module):
    ...
    def encode_image(self, videos: torch.Tensor) -> torch.Tensor:
        ...
        pixel_values = (
            pixel_values - CLIP_IMAGE_MEAN.to(device=pixel_values.device, dtype=pixel_values.dtype)
...
def _init_parallel_env(
    rank: int, world_size: int, tp_size: int, cfg_parallel_size: int, master_port: int
) -> torch.device:
    ...
    *,
    ...
    if actual_f.shape != expected_f.shape:
        raise AssertionError(
            f"{name}: shape mismatch actual={tuple(actual_f.shape)}, expected={tuple(expected_f.shape)}"
        )
...
def _sync_module(vllm_module: torch.nn.Module, dreamzero_module: torch.nn.Module) -> None:
    ...
    dreamzero_params = {name_map.get(name, name): param for name, param in dreamzero_params_raw.items()}
...
def _build_vllm_pipeline(transformer: nn.Module, device: torch.device) -> DreamZeroPipeline:
    ...
    pipe.action_norm_stats = {
        name: {key: value.clone() for key, value in stats.items()} for name, stats in ACTION_STATS.items()
    }
    ...
    *,
    ...
    for layer_idx, (actual, expected) in enumerate(zip(actual_crossattn, expected_crossattn, strict=True)):
        ...
        _assert_close(
            ...
            actual["k"],
            _slice_heads(expected["k"], tp_rank, tp_size, head_dim=2).to(
                device=actual["k"].device, dtype=actual["k"].dtype
            ),
            max_error,
            atol=STATE_CACHE_ATOL,
            rtol=STATE_CACHE_RTOL,
        ...
        _assert_close(
            ...
            actual["v"],
            _slice_heads(expected["v"], tp_rank, tp_size, head_dim=2).to(
                device=actual["v"].device, dtype=actual["v"].dtype
            ),
            max_error,
            atol=STATE_CACHE_ATOL,
            rtol=STATE_CACHE_RTOL,
