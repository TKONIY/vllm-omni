...
This file verifies that `tests/dreamzero/test_client_AR.py` preserves the same
...
THIRD_PARTY_DREAMZERO = Path(__file__).resolve().parents[2] / "third_party" / "dreamzero"
CLIENT_SCRIPT = Path(__file__).resolve().with_name("test_client_AR.py")

if str(THIRD_PARTY_DREAMZERO) not in sys.path:
    sys.path.insert(0, str(THIRD_PARTY_DREAMZERO))
