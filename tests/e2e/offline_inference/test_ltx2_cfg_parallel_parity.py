...
def test_ltx2_t2v_cfg_parallel(tmp_path: Path):
    ...
    cmd = [
        sys.executable, str(T2V_EXAMPLE),
        "--model", _get_ltx2_model(),
        "--prompt", T2V_PROMPT,
        "--negative-prompt", T2V_NEGATIVE_PROMPT,
        "--height", "256", "--width", "256",
        "--num-frames", "145", "--num-inference-steps", "6",
        "--guidance-scale", "4.0",
        "--frame-rate", "24", "--fps", "24", "--seed", "42",
        "--cfg-parallel-size", "2", "--enforce-eager",
        "--output", str(output),
    ]
...
def test_ltx2_i2v_cfg_parallel(tmp_path: Path):
    ...
    env.setdefault("CUDA_VISIBLE_DEVICES", "0,1")
    cmd = [
        sys.executable, str(I2V_EXAMPLE),
        "--model", _get_ltx2_model(),
        "--model-class-name", "LTX2ImageToVideoPipeline",
        "--image", str(test_image),
        "--prompt", I2V_PROMPT,
        "--negative-prompt", I2V_NEGATIVE_PROMPT,
        "--height", "256", "--width", "256",
        "--num-frames", "73", "--num-inference-steps", "6",
        "--guidance-scale", "4.0",
        "--frame-rate", "24", "--fps", "24", "--seed", "42",
        "--cfg-parallel-size", "2", "--enforce-eager",
        "--output", str(output),
