...
def _run_worker(
    ...
    with set_current_vllm_config(cfg):
        ...
        try:
            ...
            if local_rank == 0:
                print(
                    f"RowParallelLinear precision check: dtype={dtype_name}, "
                    f"tp={world_size}, max_diff={diff:.6e}"
                )

            if expect_drift:
                assert diff >= BF16_DRIFT_MIN, (
                    f"Expected native bf16 TP RowParallelLinear drift >= {BF16_DRIFT_MIN:.3e}, "
                    f"got {diff:.3e}"
                )
            else:
