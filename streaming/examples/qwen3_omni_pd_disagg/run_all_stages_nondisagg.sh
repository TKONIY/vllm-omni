#!/usr/bin/env bash
# Orchestrate the three non-disagg Qwen3-Omni stages, each in its own
# top-level Python process so each one can be wrapped in its own
# debugpy listener.
#
# Counterpart of run_all_stages_disagg.sh, but for the 3-stage non-PD
# layout in qwen3_omni_a5000.yaml (thinker prefill+decode merged):
#
#   stage 0 (thinker, prefill+decode + API)  -> 5678
#   stage 1 (talker)                         -> 5679
#   stage 2 (code2wav)                       -> 5680
#
# Usage:
#   ./run_all_stages_nondisagg.sh                  # no debug, all three stages
#   ./run_all_stages_nondisagg.sh --debug          # all three stages wait for VSCode
#   ./run_all_stages_nondisagg.sh --debug --debug-stages 0,1
#                                                  # only stages 0 and 1 wait
#
# Behaviour:
#   * Stages 1, 2 run in the background; their stdout/stderr stream to
#     /tmp/vllm_omni_nondisagg_stage<N>.log.
#   * Stage 0 runs in the foreground; Ctrl-C tears down all three.
#   * In --debug mode, every selected stage uses wait-for-client. Attach
#     them via the "Compound: Attach all non-disagg stages (0..2)" launch
#     in VSCode (or attach each port individually).
...
DEBUG_HOST="0.0.0.0"
# stage_id -> port
DEBUG_PORTS=(5678 5679 5680)

LOG_DIR="${VLLM_OMNI_NONDISAGG_LOG_DIR:-/tmp}"
...
Usage: $0 [OPTIONS]
  ...
  --stage-configs PATH    Non-disagg stage configs YAML (default: ${STAGE_CONFIGS})
  ...
  --debug-ports LIST      Comma-separated debugpy ports for stages 0..2
                          (default: ${DEBUG_PORTS[*]})
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --debug-ports)
            ...
            if [[ ${#DEBUG_PORTS[@]} -ne 3 ]]; then
                echo "Error: --debug-ports needs 3 comma-separated values (stages 0..2)" >&2
                exit 1
            fi
            shift 2 ;;
...
echo "=========================================="
echo "vLLM-Omni non-disagg — 3-stage launch"
echo "=========================================="
echo "Model:         $MODEL"
...
if [[ "$DEBUG" == "true" ]]; then
    ...
    echo "Debug ports:   ${DEBUG_PORTS[*]}  (stage 0..2)"
fi
echo "=========================================="

# Start stages 1, 2 in the background first so stage 0 (which holds
# the API socket and prints to the foreground) is the last to come up.
for sid in 1 2; do
    cmd=()
    build_stage_cmd "$sid" cmd
    log="${LOG_DIR}/vllm_omni_nondisagg_stage${sid}.log"
    echo ""
