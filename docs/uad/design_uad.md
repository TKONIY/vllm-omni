...
- runner 不识别模型私有 token，不调用模型状态机。runner 只根据 scheduler output 构造
  batch、执行统一的 HunyuanImage3UADModel、执行 logits/sampling 后返回 raw output。
...
- `ar_sampler_token_ids`：AR sampler 的 generated-token history，用来填充 vLLM
  `SamplingMetadata.output_token_ids`。它不是 `engine_tokens[prompt_len:]`，因为
  `engine_tokens` 未来会包含 image context placeholder、DiT commit token 等多模态数据。
  它也不是对外输出；图片输出后续走 `materialized_tokens` / artifact 结构。
...
- `ar_sampler_token_ids`：调度时 request 的 AR sampler history snapshot。
- `sample_token_offset`：本 item 内哪个 token 的 hidden state 需要投影到 logits，对齐
  vLLM `logits_indices`。当前 AR MVP 是最后一个 scheduled token。
...
- 在模型 forward 后执行 `compute_logits()` 和 `sample()`，对齐 vLLM
  `GPUModelRunner` 的职责边界。
...
- 它返回 hidden states / denoise prediction，不做 logits projection、sampling 或 request 更新。
...
当前 Milestone B 的真实 AR backend path 仍是 smoke path：

- `HunyuanImage3UADModel` 只调用 backend `forward(input_ids, positions)`。
- `UADRunner` 调用 backend `compute_logits()` 和 `sample()`。
- 只支持单 AR item；多请求真实 batch 要等 paged KV / attention metadata 接入。
...
`new_ar_sampler_token_ids` 只更新 AR sampler history，用于后续 vLLM
`SamplingMetadata.output_token_ids`。它不是 `new_materialized_tokens`，也不会承载 image
artifact。
