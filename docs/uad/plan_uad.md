...
| `UADModelStateMachine` | 模型私有 phase/output-ledger 规则 | 不调用 runner；不调度 |
...
| Phase | KV 行为 |
|---|---|
| AR prefill/decode | 写 vLLM paged KV；推进 `num_computed_tokens` |
| DiT non-final denoise | 不写 paged KV；只更新 diffusion runtime state |
| DiT final image-context commit | 写 vLLM paged KV；推进到 image context 后 |
...
- final commit 设计明确，暂未强制完成真实 paged KV commit。

## 7. Step 5：Final image-context KV commit

目标：让 final DiT step 默认把 generated image context 写入 vLLM paged KV，原生支持
multiturn。

范围：

- scheduler 表达 image-context commit item，并在 forward 前用 vLLM `KVCacheManager`
  分配 image tokens 的 slots。
- runner 根据 HunyuanImage3 image grid 生成 image context embeddings / positions，并写入
  paged KV。
- state machine 推进 `num_computed_tokens` 到 image context 之后。
- VAE/artifact decode 仍不进入 scheduler token budget。

验证：

- final commit 后 `num_computed_tokens` 覆盖 image context tokens。
